    ...
    switch (country) {
          ...
          { key: "docs", label: isAr ? "المستندات" : "Documents", content: (
            <div className="space-y-4">
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    {[{ name: "CR_Document.pdf", status: "verified" }, { name: "VAT_Certificate.pdf", status: "pending" }].map((doc) => (
                      <div key={doc.name} className="flex items-center justify-between rounded-lg border p-3">
                        ...
                        <Badge variant={doc.status === "verified" ? "default" : "secondary"}>
                          {doc.status === "verified" ? <CheckCircle2 className="size-3 mr-1" /> : <AlertTriangle className="size-3 mr-1" />}
                          {doc.status}
                        </Badge>
                      </div>
                    ))}
      ...
      case "AE":
        return [
          ...
          { key: "docs", label: isAr ? "المستندات" : "Documents", content: (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">{isAr ? "المستندات المرفوعة" : "Uploaded documents"}</p>
              <div className="space-y-2">
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <div className="flex items-center gap-2">
                    <FileText className="size-4 text-blue-600" />
                    <span className="text-sm">Trade_License.pdf</span>
                  </div>
                  <Badge><CheckCircle2 className="size-3 mr-1" />verified</Badge>
                </div>
              </div>
            </div>
          )},
