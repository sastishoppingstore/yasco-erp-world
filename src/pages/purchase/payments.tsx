...
export default function SupplierPaymentsPage() {
  const { data: payments, refetch } = trpc.purchase.supplierPaymentList.useQuery();
  const { data: suppliers } = trpc.purchase.supplierList.useQuery();
  const createPayment = trpc.purchase.supplierPaymentCreate.useMutation({ onSuccess: () => refetch() });
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({
    paymentNumber: "", supplierId: 0, date: "", amount: "0",
    paymentMethod: "cash" as const, reference: "", notes: "",
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl font-bold">Supplier Payments</h2><p className="text-slate-500">Track supplier payments</p></div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <form onSubmit={(e) => { e.preventDefault(); createPayment.mutate(form); setOpen(false); setForm({ paymentNumber: "", supplierId: 0, date: "", amount: "0", paymentMethod: "cash", reference: "", notes: "" }); }} className="space-y-4">
              ...
              {payments?.map(p => {
                ...
                return (
                  <TableRow key={p.id}>
                    ...
                    <TableCell>{suppliers?.find(s => s.id === p.supplierId)?.name || `Supplier #${p.supplierId}`}</TableCell>
                    ...
                    <TableCell className="text-right font-mono font-semibold text-red-600">{Number(p.amount).toLocaleString()} SAR</TableCell>
